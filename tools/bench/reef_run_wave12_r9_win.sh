...
run T8-stall bash -lc '
  ...
  export PROBE_DEV=/dev/sdd OUT=/v/xycalc-results/wave12-r9/t8-stall
  export PROBE_BASELINE_SECONDS=15 PROBE_STALL_SECONDS=20 PROBE_RECOVERY_TIMEOUT=180
  export PROBE_RATES=200 PROBE_DOCS=900000 PROBE_POLICIES=none,immediate,exponential
  export PROBE_STALL_MODE=pause
  docker compose down -v >/dev/null 2>&1 || true
  docker compose build worker driver stall-driver
  ./run_stall_recover.sh
' || true

run T4-timeseries env PROBE_MODE=timeseries PROBE_LEVELS=8 PROBE_SECONDS=480 \
  PROBE_DOCS=800000 PROBE_CACHE_GB=0.25 PROBE_DEV=/dev/sdd \
  bash tools/bench/ticket_probe.sh > "$OUT/t4-timeseries.json" || true

echo "=== wave12-r9 DONE $(date -Is) ==="
ls -la "$OUT"
