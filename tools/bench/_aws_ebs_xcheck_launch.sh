...
COST_NOTE="m6i.large us-east-2 ~\$0.10/hr + gp3 ${GP3_GIB}GiB @ ${GP3_IOPS}/${GP3_TP}; max ${MAX_HOURS}h. Soft campaign \$125 / hard \$150."
...
cost=$COST_NOTE
phases: under(~2000 IOPS 3m) then over(unthrottled high QD 3m)
...
# --- Phase UNDER: stay under provisioned 3000 IOPS ---
...
IOSTAT_PID=\$!
fio --name=under --filename="\$TEST" --direct=1 --rw=randread --bs=16k \\
  --ioengine=libaio --iodepth=8 --rate_iops=2000 \\
...
IOSTAT_PID=\$!
fio --name=over --filename="\$TEST" --direct=1 --rw=randread --bs=16k \\
...
scp -i "$STAGE/${KEY}.pem" -o StrictHostKeyChecking=no \
  "$STAGE/run_xcheck.sh" "ec2-user@${IP}:/tmp/"
"${SSH[@]}" 'sudo mkdir -p /opt/xycalc && sudo cp /tmp/run_xcheck.sh /opt/xycalc/run_xcheck.sh && sudo chown -R ec2-user:ec2-user /opt/xycalc && chmod +x /opt/xycalc/run_xcheck.sh'
