...
const XYCALC_APP = (() => {
  ...
  function attachUi() {
    ...
    function bootSimple() {
      ...
      $("simple-vulns").value = defaults.baseline_vuln_count || "";
      $("simple-vuln-storage").value = defaults.baseline_storage_size || "";
      $("simple-devices").value = "";
      $("simple-device-avg").value = "";
      $("simple-residual").value = "";
      ["simple-vulns", "simple-vuln-storage", "simple-devices",
        "simple-device-avg", "simple-residual"].forEach((id) => {
        $(id).addEventListener("input", scheduleSimpleCalc);
      });
      $("simple-open-advanced").addEventListener("click", () => setMode("advanced"));
      scheduleSimpleCalc();
    }
    ...
    function calculateSimple() {
      ...
      if ((devices && !deviceAvgRaw) || (!devices && deviceAvgRaw)) {
        ...
        err.hidden = false;
        err.textContent =
          "Device count and avg on-disk bytes must be supplied together.";
        status.textContent = "Fix the device fields to size.";
        return;
      }
      const storage = normalizeSimpleSize(storageRaw);
      const residual = residualRaw ? normalizeSimpleSize(residualRaw) : "";
      const deviceAvg = deviceAvgRaw ? normalizeSimpleAvgBytes(deviceAvgRaw) : "";
      // Same corpus chain as Advanced — Simple answers "today's footprint":
      // target == baseline so NVD compound growth does not inflate a measured
      // vuln size; no demo index/foreign pads. Advanced is where those live.
      const inputs = {
        baseline_vuln_count: vulns,
        baseline_storage_size: storage,
